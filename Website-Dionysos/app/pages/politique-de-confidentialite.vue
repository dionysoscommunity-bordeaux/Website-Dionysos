...
const { data, error, pending } = await useFetch<{ ok: boolean, data: LegalContent }>('/api/legal/politique-de-confidentialite')
const legal = computed(() => data.value?.data)

usePageSeo({
  title: 'Politique de confidentialité',
  description: 'Gestion des données personnelles et droits utilisateurs sur Dionysos Bordeaux.',
...
<template>
  <main class="mx-auto w-full max-w-4xl px-6 py-12 md:py-16">
    <h1 class="text-3xl font-bold text-[#7be9ff] md:text-4xl">{{ legal?.title || 'Politique de confidentialité' }}</h1>
