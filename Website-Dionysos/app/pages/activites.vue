...
const upcomingEvents = computed(() => calendarData.value?.data || [])
...
const formatCalendarDate = (input: string) =>
...
usePageSeo({
  title: 'Nos Sorties',
  description: 'Retrouvez toutes les sorties a venir et les types de sorties Dionysos a Bordeaux sur une seule page.',
  path: '/activites'
})
</script>
  ...
  <main class="relative min-h-screen overflow-x-hidden bg-[#070016] text-slate-100">
    ...
    <section class="diony-reveal relative mx-auto w-full max-w-6xl px-6 pb-10 pt-14 md:pb-12 md:pt-20">
      ...
      <h1 class="max-w-3xl bg-gradient-to-r from-[#23d9ff] via-[#7a2cff] to-[#ff2bb5] bg-clip-text text-4xl font-extrabold leading-tight text-transparent md:text-6xl">
        Toutes les sorties a venir et leurs formats.
      </h1>

      <p class="mt-5 max-w-3xl text-lg text-slate-200 md:text-xl">
        Cette page regroupe le calendrier des prochaines sorties et les types de formats proposes par la communaute.
      </p>
    ...
    <section class="diony-reveal diony-reveal-delay-1 relative mx-auto w-full max-w-6xl px-6 py-10 md:py-12">
      ...
      <p v-if="calendarPending" class="rounded-xl border border-white/10 bg-[#0b0322]/60 p-4 text-slate-300">
        Chargement du calendrier...
      </p>

      <div v-else-if="calendarError" class="rounded-xl border border-rose-300/30 bg-rose-300/10 p-4 text-rose-200">
        <p class="font-semibold">Le calendrier est temporairement indisponible.</p>
        <p class="mt-1 text-sm text-rose-100/90">
          Verifie la connexion Discord ou reessaie dans quelques instants.
        </p>
      </div>

      <div v-else-if="upcomingEvents.length === 0" class="rounded-xl border border-white/10 bg-[#0b0322]/60 p-4 text-slate-300">
      ...
      <div v-else class="grid gap-4 md:grid-cols-6">
        ...
        >
          ...
          <p class="mt-2 text-sm text-slate-200">{{ formatCalendarDate(item.startAt) }}</p>
          <p v-if="item.endAt" class="text-xs text-slate-400">Fin: {{ formatCalendarDate(item.endAt) }}</p>
          <p class="mt-3 text-sm text-slate-300">{{ item.description || 'Details disponibles dans la sortie Discord.' }}</p>
    ...
    <section class="diony-reveal diony-reveal-delay-1 relative mx-auto w-full max-w-6xl px-6 py-10 md:py-12">
      ...
      <p class="md:col-span-2 text-sm text-slate-300">
        Chaque type de sortie te montre clairement la fréquence, le format et l'ambiance.
      </p>
      <article
